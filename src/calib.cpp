    ...
    for( i = j = 0; i < nimages; i++ )
    {
        for( k = 0; k < 2; k++ )
        {
            ...
            if( displayCorners )
            {
                ...
                drawChessboardCorners(cimg, boardSize, corners, found);compute
                double sf = 640./MAX(img.rows, img.cols);compute
                resize(cimg, cimg1, Size(), sf, sf, INTER_LINEAR_EXACT);compute
                imshow("corners", cimg1);compute
                char c = (char)waitKey(500);compute
                if( c == 27 || c == 'q' || c == 'Q' ) //Allow ESC to quitcompute
                    exit(-1);
            }
            else
                putchar('.');
...
// epipolar geometry constraint: m2^t*F*m1=0
    ...
    if( fs.isOpened() )
    {
        fs << "Ml" << cameraMatrix[0] << "Dl" << distCoeffs[0] <<
            "Mr" << cameraMatrix[1] << "Dr" << distCoeffs[1];
        fs.release();
    }
    else
...
int main(int argc, char** argv)
{
    ...
    vector<string> imagelist;n
    bool ok = readStringList(imagelistfn, imagelist);n
    if(!ok || imagelist.empty())n
    {n
        cout << "can not open " << imagelistfn << " or the stnring list is empty" << endl;
        return print_help();
    }
